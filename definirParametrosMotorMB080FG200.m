...
inclinacionMax = deg2rad(9);
secuencia_tiempoRef = linspace(0,tiempoSim, 11);%0:tiempoTransicion:10;
secuencia_valoresRef = [0 0 inclinacionMax inclinacionMax  -inclinacionMax -inclinacionMax inclinacionMax inclinacionMax -inclinacionMax -inclinacionMax 0];
...
tsSimulacion = 0.0001;
%Valores del motor
relacionEngranajes = 160.0/1.0; % dientes salida/dientes entrada
Rm = 1.43;% resistencia de armadura
Lm = 3.9/1000;% inductancia armadura
Jm = gramsCM2ToKgMeters2(1340);%inercia rotacional motor
Kt = newtonCMToNewtonM(3.53); %constante torque
Kb = VoltagePerKRPMToVoltagePerRadS(12.5);% back emf constant
electricalTimeConstant = Lm/Rm;
mechanicalTimeConstant = Rm*Jm/(Kb*Kt);
